...
const AssignStudentClass: FC<Props> = ({ onClose, data: toUpdate, refetch }) => {
  ...
  const handleAssignClass = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const res = await AuthApi.put('/students/assign/class', data, {
        params: data,
      });

      notifications.show({
        title: 'Classes Assigned',
        message: 'Classes have been assigned successfully',
        color: 'green',
      });
      refetch();
      onClose();
  ...
  return (
    <form className=" w-full flex flex-col gap-y-3" onSubmit={handleAssignClass}>
      <h1 className=" text-center mt-4 font-semibold text-sm">
        Assign Classes to {toUpdate?.firstName} {toUpdate?.lastName}
      </h1>
