...
const TermClassAcademicSort = ({ setData, data: defaultData }: Props) => {
  const [acaYearId, setAcadYearId] = useState('');
  const [termId, setTermId] = useState('');
  const [classId, setClassId] = useState('');
  const { data: classes } = useGet<IClass[]>('/classes/all/current-year', { defaultData: [] }); // TODO: get classes by academic year and term
  const { data, loading, error, get } = useGet<any[]>(
    `/student-class-term/class/term
`,
    {
      query: {
        classId,
    ...
    <div className="flex flex-col w-full">
      <div className="flex items-center justify-between gap-3 w-full">
        ...
        <div className=" gap-x-3 flex items-center">
          <span className=" px-2 text-gray-600">View By</span>
          <Select
            // label="View"
            placeholder="View By"
            size="sm"
      ...
      {loading && <span className=" text-center">Applying filters ...</span>}
    </div>
  );
};

export default TermClassAcademicSort;
