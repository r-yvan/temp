...
const AdminParentStudent = () => {
  ...
  const columns: ColumnDef<Student>[] = [
    ...
    },
    {
      header: 'Assign Class',
      cell: ({ row }) => (
        <ActionIcon variant="transparent" onClick={() => onAssignClass(row.original)}>
          <FaTasks className={'text-[#475FDE]'} size={20} />
        </ActionIcon>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-x-2">
          ...
          <Link href={`/admin/students/edit/${row.original.id}`}>
            <EditIcon />
          </Link>
          <ActionIcon variant="transparent" onClick={() => setIsOpen(true)}>
            <DeleteIcon />
          </ActionIcon>
        </div>
      ),
