...
const AdminCustomization: React.FC = () => {
  ...
  const onSubmit = (data: any) => {
    ...
    axios
      .post(`${backend}/web-content/create`, formData, {
        headers: {
  ...
  const handleFilesSelected = (filetype: string, files: File[]) => {
    ...
    setImages((prevImages) => (prevImages ? [...prevImages, newImage] : [newImage]));
  };

  return (
    <form
      onSubmit={handleSubmit(onSubmit)}
      className="w-full h-full overflow-y-auto overflow-x-hidden p-2 text-sm"
    >
      ...
      <div className="my-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid-rows-2 gap-3">
        <FileDropZone
          fileType="landingBurnerImage"
          onFilesSelected={handleFilesSelected}
          title="Main Landing Page Barner Image"
        />
        <FileDropZone
          fileType="abouUsImage"
          onFilesSelected={handleFilesSelected}
          title="Main Landing About Us Large Image"
        />
        {/* <FileDropZone fileType='aboutSmall' onFilesSelected={handleFilesSelected} title='Main Landing About Us Smaller Image' /> */}
        <FileDropZone
          fileType="innovationMainImage"
          onFilesSelected={handleFilesSelected}
          title="Innovations Images maximum 10 shots"
        />
        <FileDropZone
          fileType="newsLetterCardImage"
          onFilesSelected={handleFilesSelected}
          title="News Letter small Card Image"
        />
        <FileDropZone
          fileType="newsLetterLargeImage"
          onFilesSelected={handleFilesSelected}
          title="News Letter large Card Image"
        />
      </div>
      <div className="my-10 flex flex-col md:flex-row gap-2 md:gap-5">
        <div className="w-full md:w-[50%]">
          ...
          <textarea
            ...
            placeholder="Enter About Section Text appearing on the landing page"
            className="text-black  bg-[rgba(67,67,67,0.03)]  rounded-md border-[2px] border-[rgba(67,67,67,0.09)] w-full px-3 py-2"
          ></textarea>
        </div>
        <div className="w-full md:w-[50%]">
          <p className="text-[rgba(67,67,67,0.43)] my-2">Innovation Text</p>
          <textarea
            {...register('innovationText')}
            placeholder="Enter Innovation section Text appearing on the landing page"
            className="text-black  bg-[rgba(67,67,67,0.03)]  rounded-md border-[2px] border-[rgba(67,67,67,0.09)] w-full px-3 py-2"
          ></textarea>
        </div>
      </div>
      <div className="my-10">
        ...
        <div className="my-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2       ">
          <input
            {...register('yearsFoundation')}
            type="number"
            placeholder="Years of Foundation"
            className=" px-3 py-2 text-black  bg-[rgba(67,67,67,0.03)]  rounded-md border-[2px] border-[rgba(67,67,67,0.09)]"
          />
          <input
            {...register('stakePartener')}
            type="number"
            placeholder="Stakeholders and Partners"
            className=" px-3 py-2 text-black  bg-[rgba(67,67,67,0.03)]  rounded-md border-[2px] border-[rgba(67,67,67,0.09)]"
          />
          <input
            {...register('students')}
      ...
      <div className="my-10">
        ...
        <div className="my-2 grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input
            {...register('slackLink')}
            type="text"
            placeholder="Slack Workspace Link"
            className=" px-3 py-2 text-black  bg-[rgba(67,67,67,0.03)]  rounded-md border-[2px] border-[rgba(67,67,67,0.09)]"
          />
          <input
            {...register('discordServer')}
            type="text"
          ...
          <input
            {...register('github')}
            type="text"
            placeholder="Github Open RCA Link"
            className=" px-3 py-2 text-black  bg-[rgba(67,67,67,0.03)]  rounded-md border-[2px] border-[rgba(67,67,67,0.09)]"
          />
          {/* <input {...register("email")} type="text" placeholder='SnapChat Contact' className=' px-3 py-2 text-black  bg-[rgba(67,67,67,0.03)]  rounded-md border-[2px] border-[rgba(67,67,67,0.09)]' /> */}
        </div>
      </div>
      <div className="my-10 flex flex-row gap-10">
        <button className="bg-[rgba(67,67,67,0.03)]  text-black rounded-md  border-[2px] border-[rgba(67,67,67,0.09)] px-5 py-2">
          Cancel Changes
        </button>
        <div>
          {!loading ? (
            ...
            <div className="bg-primary rounded-md text-white px-5 py-2 cursor-wait">
